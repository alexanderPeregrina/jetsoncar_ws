...
class ControlNode(object):
	...
	self.actions = np.linspace(-45, 45, self.nb_actions)
        self.action = 0
	#self.Q = np.load('Q_15_15_11_left.npy')
	self.eta_values = np.linspace(-35, 35, 15)
	self.delta_x_values = np.linspace(-35, 35, 15)
	#Model 1
	self.Q1 = np.load('Q_15_15_11.npy')

	#Model 2
	self.Q2 = np.load('Q_15_15_11_left.npy')
	...
	counter_left = 0
        ...
        while not rospy.is_shutdown():
		    ...
		    if self.model_number == 1:
		    	...
		    	self.action = self.actions[np.argmax(self.Q1[idx1, idx2, :])]
		    	self.twist.angular.z = self.action 
			
		    	
		    else:
			self.action = self.actions[np.argmax(self.Q2[idx1, idx2, :])]
		    	self.twist.angular.z = self.action
